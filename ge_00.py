...
def search_faq(question):
    """
    從 Google Sheets FAQ 表格中查找問題的回答
    """
    try:
        # 獲取 FAQ 表格中的所有數據
        all_values = faq_sheet.get_all_values()
        ...
        # 遍歷 FAQ 表格，查找匹配的問題
        for row in all_values[1:]:  # 跳過表頭
            stored_question = row[0]  # 假設問題在第一列
            stored_answer = row[1]  # 假設回答在第二列

            if stored_question and question.lower() in stored_question.lower():
                return stored_answer  # 找到匹配問題，返回對應的回答
    except Exception as e:
        logger.error(f"搜尋 FAQ 表格時出錯: {str(e)}")
    
    return ""

def search_temp(question):
    """
    從 Google Sheets 暫存表格中查找問題的回答
    """
    try:
        # 獲取暫存表格的所有數據
        all_values = chat_sheet.get_all_values()
        ...
        # 遍歷暫存表格，查找匹配的問題
        for row in all_values[1:]:  # 跳過表頭
            stored_url = row[1]  # 假設 URL 在第二列
            stored_content = row[2]  # 假設內容在第三列

            if stored_url and question.lower() in stored_url.lower():
                return stored_content  # 找到匹配的網址，返回對應的內容
    except Exception as e:
        logger.error(f"搜尋暫存表格時出錯: {str(e)}")
...
def ask_question(question):
    ...
    # 如果從 FAQ 中找不到，接著從暫存表格查找
    if not answer:
        answer = search_temp(question)
    
    # 如果兩個表格都找不到答案，可以返回一個提示
    if not answer:
        answer = "對不起，我無法回答這個問題。"
...
def chat():
    if request.method == "POST":
        ...
        answer = ask_question(question)  # 調用 ask_question 來查詢答案
