...
def new_post():
    ...
    if request.method == 'POST':
        ...
        conn = get_db_connection()
        cur = conn.cursor()
        cur.execute("INSERT INTO posts (title, content, created_at, user_id) VALUES (%s, %s, %s, %s)",
...
def interact():
    ...
    LEFT JOIN (
        SELECT user, MAX(uploadtime) AS latest
        FROM images
        GROUP BY user
    ) latest_img ON latest_img.user = Users.account
    LEFT JOIN images ON images.user = latest_img.user AND images.uploadtime = latest_img.latest
