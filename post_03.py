...
def new_post():
    ...
    if request.method == 'POST':
        ...
        cur.execute("""
            ...
            VALUES (%s, %s, %s, %s, '0', %s)
        """, (title, content, created_at, user_id, hashtag_id))
        conn.commit()
    ...
    cur.close()
    conn.close()
    return render_template('new_post.html', name=name, avatar=avatar, hashtags=hashtags)
...
def interact():
    ...
    cur.execute("""
    SELECT posts.*, Users.user_name, images.file_path, hashtag.hashtag_name
    FROM posts
...
""", ('%' + keyword + '%', '%' + keyword + '%', '%' + keyword + '%'))
        ...
        cur = conn.cursor(dictionary=True)
        cur.execute("""
    SELECT posts.*, Users.user_name, hashtag.hashtag_name
    FROM posts
    JOIN Users ON posts.user_id = Users.user_id
    LEFT JOIN hashtag ON posts.hashtag_id = hashtag.hashtag_id
    ORDER BY posts.created_at DESC
